...
petall_sums = [0.0, 0.0, 0.0]
petalw_sums = [0.0, 0.0, 0.0]
for i in range(30):
    if(iris_df['target'][i] == 0):
        
        
        petall_sums[0] += iris_df['petal length (cm)'][i] **2
        petalw_sums[0] += iris_df['petal width (cm)'][i]**2
    if(iris_df['target'][i] == 1):
    
       
        petall_sums[1] += iris_df['petal length (cm)'][i]**2
        petalw_sums[1] += iris_df['petal width (cm)'][i]**2
    if(iris_df['target'][i] == 2):
        
        
        petall_sums[2] += iris_df['petal length (cm)'][i]**2
        petalw_sums[2] += iris_df['petal width (cm)'][i]**2
...
petal_sumxy = [0.0, 0.0, 0.0]

for i in range(30):
    if(iris_df['target'][i] == 0):
        
        
        petal_sumxy[0] += iris_df['petal length (cm)'][i] * iris_df['petal width (cm)'][i]
        
    if(iris_df['target'][i] == 1):
    
       
        petal_sumxy[1] += iris_df['petal length (cm)'][i] * iris_df['petal width (cm)'][i]
    if(iris_df['target'][i] == 2):
        
        
        petal_sumxy[2] += iris_df['petal length (cm)'][i] * iris_df['petal width (cm)'][i]
...
for i in range(3):
    num = (target_num[i] * petal_sumxy[i]) - (petall_sum[i] * petalw_sum[i])
    denom = (target_num[i] * petall_sums[i]) - (petall_sum[i] ** 2)
    inclin[i] = num / denom if denom != 0 else 0

intercept = [petalw_avg[i] - (inclin[i] * petall_avg[i]) for i in range(3)]

diff = [0,0,0]

for i in range(3):
    print(f"Species {i}: y = {inclin[i]:.2f} * x + {intercept[i]:.2f}")
...
for i in range(30,50):
    ...
    if(iris_df['target'][i] == 0):
        ...
        diff[0] += (iris_df['petal width (cm)'][i] - (iris_df['petal length (cm)'][i] * inclin[0] + intercept[0]))**2
        
    if(iris_df['target'][i] == 1):
    
       
        diff[1] += (iris_df['petal width (cm)'][i] - (iris_df['petal length (cm)'][i] * inclin[1] + intercept[1]))**2
    if(iris_df['target'][i] == 2):
        
        
        diff[2] += (iris_df['petal width (cm)'][i] - (iris_df['petal length (cm)'][i] * inclin[2] + intercept[2]))**2
